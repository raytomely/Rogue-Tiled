	...
	case MAXSTICKS:			/* Special case for vorpal weapon */
	{
		...
		if ((tp = moat(y, x)) != NULL)
		{
			...
			else if (which_one == WS_POLYMORPH)
			{
				...
				if (see_monst(tp))
					//mvaddch(y, x, monster);
					move_and_draw_tile(y, x, monster);
				tp->t_oldch = oldch;
				tp->t_pack = pp;
			...
			else
			{
				if (see_monst(tp))
					...
					move_and_draw_tile(y, x, tp->t_oldch);
				if (which_one == WS_TELAWAY)
				{
					...
					if (see_monst(tp))
						...
						move_and_draw_tile(tp->t_pos.y, tp->t_pos.x, tp->t_disguise);
	...
	char *name;
{
	...
	for (i = 0; i < BOLT_LENGTH && !used; i++) {
		...
		default:
			...
			move_and_draw_tile(pos.y, pos.x, dirch);
			standend();
			break;
		}
	}
	for (j = 0; j < i; j++) {
		...
		if (spotpos[j].s_under)
			...
			move_and_draw_tile(spotpos[j].s_pos.y, spotpos[j].s_pos.x, spotpos[j].s_under);
	}
}
