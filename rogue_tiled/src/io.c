	...
	char *str1, *str2;
{
	...
	if (str1 == 0) {
		...
		while (str1 <= str2) {
			move(msgline,0);//printf("while_scrlmsg str1= 0x%x str2= 0x%x\n",str1, str2);
			//printf("str1_len=%d cols=%d s=%d \n",strlen(str1), COLS, (signed)strlen(str1));
            //SDL_FillRect(screen, NULL, SDL_MapRGB(screen->format, 255, 255, 0));
			printw(fmt,str1++);printf(fmt);
...
extern bool capslock, numlock, scrollock;
/*
 * key_state:
 */
void
SIG2(void)
{
	...
	new_numl &= 0x20;*/
    new_capsl = capslock;
	new_fmode = scrollock;  //@ scroll lock
	new_numl  = numlock;
#ifdef ROGUE_DOS_CLOCK
	/*
